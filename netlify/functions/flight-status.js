...
exports.handler = async function (event) {
  ...
  try {
    ...
    //
    // IMPORTANT: AeroDataBox can pre-populate "actualTime"/"runwayTime" fields with
    // an ESTIMATE well before the flight has actually moved — so their mere presence
    // isn't proof the event really happened. The overall flight status text (e.g.
    // "Expected" vs "Departed" vs "Landed") is the authoritative signal, so locking
    // is gated on that instead.
    function computeLeg(leg, hasReallyHappened) {
      leg = leg || {};
      const scheduledStr = extractLocalTimeString(leg.scheduledTime) || extractLocalTimeString(leg.scheduledTimeLocal) || null;
      const actualStrRaw = extractLocalTimeString(leg.actualTime) || extractLocalTimeString(leg.runwayTime) ||
      ...
      // Only trust the "actual" time as genuinely real if the overall flight status
      // confirms this leg has really happened — otherwise treat it as just another estimate.
      const actualStr = hasReallyHappened ? actualStrRaw : null;
      const locked = !!actualStr;
      const bestStr = actualStr || revisedStr || actualStrRaw || scheduledStr;
      const time = formatLocalTime(bestStr);

      let status = 'unknown';
      const compareStr = actualStr || revisedStr || actualStrRaw;
    ...
    // What AeroDataBox actually confirms has happened, based on the overall status text —
    // not still "expected/scheduled/checkin/boarding/gateclosed/delayed" (all still on the ground).
    const departureReallyHappened = /departed|enroute|landed|arrived|diverted|approach/.test(rawStatus);
    const arrivalReallyHappened = /landed|arrived/.test(rawStatus);

    const departure = computeLeg(entry.departure, departureReallyHappened);
    const arrival = computeLeg(entry.arrival, arrivalReallyHappened);
